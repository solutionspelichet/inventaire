...
async function tryManualFocus(distance = 0.1) {
  ...
  try { await videoTrack.applyConstraints({ advanced: [{ focusMode: 'manual', focusDistance: d }] }); return true; } catch { return false; }
}
...
async function openStream(){
  ...
  const base = {
    video: {
      ...
      width:  { ideal: wantW, max: wantW },
      height: { ideal: wantH, max: wantH },
  ...
  try {
    ...
    try {
      return await navigator.mediaDevices.getUserMedia({
        video: { deviceId: deviceToUse?{exact:deviceToUse}:undefined, width:{ideal:1920,max:1920}, height:{ideal:1080,max:1080}, frameRate:{ideal:30,max:30} },
        audio:false
      });
    } catch(e2) {
      return await navigator.mediaDevices.getUserMedia({
...
els.btnTorch.addEventListener('click', async ()=>{
  ...
  else alert("Lampe non supportée par cette caméra (le mode 'écran blanc' s'active si besoin en éclairant l'environnement).");
});
els.zoomRange.addEventListener('input', e=> setZoom(e.target.value));

// Double-tap pour zoom 1→2→3→1
let lastTap=0;
els.videoWrap.addEventListener('touchend', ()=>{
  const now=Date.now();
  if (now-lastTap<300){
    const cur=Number(els.zoomRange.value), max=Number(els.zoomRange.max||3);
    let next = cur<1.8 ? 2 : (cur<2.8 ? 3 : 1);
    next = Math.min(next, max);
    els.zoomRange.value = next; setZoom(next);
  }
  lastTap=now;
},{passive:true});
...
async function setupDetector(){
  ...
  try{ const supported=await BarcodeDetector.getSupportedFormats(); const list=formats.filter(f=>supported.includes(f)); if (!list.length) return false; detector=new BarcodeDetector({formats:list}); return true; }catch{ return false; }
}
...
function startQuagga(){
  ...
  setTimeout(()=>{
    Quagga.init({
      inputStream:{ name:'Live', type:'LiveStream', target:els.videoWrap,
...
async function start(){
  try{
    ...
    // Macro: zoom initial + focus proche si possible
    if (macroMode){
      if (!els.zoomWrap.hidden){
        const targetZoom = Math.min(Number(els.zoomRange.max||3), 2.0);
        els.zoomRange.value = targetZoom; await setZoom(targetZoom);
      }
      await tryManualFocus(0.1);
    }

    const nativeOk = await setupDetector();
    if (selectedEngine==='native' || (selectedEngine==='auto' && nativeOk)){ startNativeLoop(); }
    else if (selectedEngine==='zxing' || (selectedEngine==='auto' && !nativeOk)){ startZXing(); }
    else { startQuagga(); }

    if (selectedEngine!=='quagga'){
      const t = is1DMode() ? 2500 : 4000;
      fallbackTimer=setTimeout(()=>{ if (!usingQuagga && els.codeType.value!=='QR_CODE') startQuagga(); }, t);
    }
